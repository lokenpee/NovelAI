...
export function renderWorkspace(project = {}, { openChapterId = null, openCategoryIds = new Set(), openEntryIds = new Set(), isProcessing = false } = {}) {
  ...
  return `<div class="nai-workspace">${section('文件上传', '📄', fileBody, fileActions)}${section('清洗重复段落', '🧹', cleanBody)}${section('章回正则设置', '📖', regexBody)}${section('章节队列', '📋', queueBody)}${renderWorldbook(project, { openCategoryIds, openEntryIds, isProcessing })}</div>`;
}
